    ...
    while (true) {
        try {
            ...
            if (mPrice) {
                ...
                log(`${lang.runningTime}${elapsedTime()}`)
                log(
                    '==========================================================='
                )
                ...
                if (!isNaN(totalProfits)) {
                    ...
                    log(
                        `${lang.withdrawalProfits}${parseFloat(
                    ...
                    logColor(
                        ...
                        `${
                            lang.realProfits
                        } [SL = ${STOP_LOSS_BOT}%, TP = ${TAKE_PROFIT_BOT}%]: ${totalProfitsPercent}% ==> ${
                            totalProfits <= 0 ? '' : '+'
                        }${parseFloat(totalProfits.toFixed(3))} ${MARKET2}`
                    )

                    if (totalProfitsPercent >= parseFloat(TAKE_PROFIT_BOT)) {
                        logColor(colors.green, lang.closingBotOnWin)
                        increaseProfitsSound()
                        if (SELL_ALL_ON_CLOSE) {
                            if (
                                WITHDRAW_PROFITS &&
                                totalProfits >= parseFloat(MIN_WITHDRAW_AMOUNT)
                            ) {
                        ...
                        logColor(colors.red, lang.closingBotOnLost)
                ...
                const entryPercent = parseFloat(
                    ((100 * entryFactor) / entryPrice).toFixed(2)
                )
                log(
                    `${lang.entryPrice} ${store.get(
                        'entry_price'
                    )} ${MARKET2} (${
                        entryPercent <= 0 ? '' : '+'
                    }${entryPercent}%)`
                ...
                log(`${lang.prevPrice} ${startPrice} ${MARKET2}`)

                if (marketPrice < startPrice) {
                    let factor = startPrice - marketPrice
                    let percent = parseFloat(
                        ((100 * factor) / startPrice).toFixed(2)
                    )

                    logColor(
                        colors.red,
                        `New price: ${marketPrice} ${MARKET2} ==> -${parseFloat(
                            percent.toFixed(3)
                        )}%`
                    )
                    ...
                    if (percent >= BUY_PERCENT)
                        await _buy(marketPrice, BUY_ORDER)
                } else {
                    ...
                    logColor(
                        ...
                        `${
                            lang.newPrice
                        } ${marketPrice} ${MARKET2} ==> +${parseFloat(
                            percent.toFixed(3)
                        )}%`
                    )
                    store.put('percent', `+${parseFloat(percent.toFixed(3))}`)
                ...
                if (orders.length > 0) {
                    ...
                    console.log()
                    log(lang.lastBuyOrder)
                    console.log('=================================')
                    log(`${lang.buyPrice} ${bOrder.buy_price} ${MARKET2}`)
                    log(`${lang.sellPrice} ${bOrder.sell_price} ${MARKET2}`)

                    if (USE_STOP_LOSS_GRID) {
                        const slStrategy = STOP_LOSS_GRID_IS_FIFO
                            ? 'FIFO'
                            : 'LIFO'
                        log(
                            `${lang.stopLossPrice} ${bOrder.sl_price} ${MARKET2}, ${lang.strategy} ${slStrategy}`
                        )
                        log(
                            `${lang.slLosses} ${parseFloat(
                                store.get('sl_losses')
                            ).toFixed(3)}, ${
                                lang.triggerPriceDown
                            } ${STOP_LOSS_GRID}%`
                        )
                    }

                    log(
                        `${lang.orderAmount} ${BUY_ORDER} ${MARKET2} ==> ${bOrder.amount} ${MARKET1}`
                    )
                    ...
                    if (expectedProfits >= 0)
                        logColor(
                            colors.green,
                            `${lang.expectedProfit} +${expectedProfits} ${MARKET2}`
                        )
                    else
                        logColor(
                            colors.red,

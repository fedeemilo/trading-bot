...
const lang = require('../constants/language')(LANGUAGE)
...
async function withdraw(profits, price) {
    await _sellAll()
    console.log(lang.processingWithdrawal)
    ...
    await client.withdraw({
        ...
        address:
            MARKET2 === 'BUSD' ? WITHDRAW_ADDRESS_BUSD : WITHDRAW_ADDRESS_USDT,
        amount: profits
    })
...
function getRealProfits(price) {
    ...
    return parseFloat(
        (
            (m1Balance - initialBalance1) * price +
            m2Balance -
            initialBalance2
        ).toFixed(4)
    )
...
const _logProfits = async price => {
    ...
    logColor(
        isGainerProfit == 1
            ...
            ? colors.red
            : colors.gray,
        `${lang.gridProfits}${parseFloat(store.get('profits')).toFixed(
            4
        )} ${MARKET2}`
    )
    ...
    logColor(
        colors.gray,
        `${lang.balance}${m1Balance} ${MARKET1}, ${m2Balance.toFixed(
            2
        )} ${MARKET2}`
    )
    logColor(
        colors.gray,
        `${lang.currentBalance}${parseFloat(
            (m1Balance * price + m2Balance).toFixed(2)
        )} ${MARKET2}, ${lang.initialBalance}${initialBalance.toFixed(
            2
        )} ${MARKET2}`
    )
}
    ...
    if (totalAmount >= parseFloat(minSell)) {
        try {
            ...
            if (res && res.status === 'FILLED') {
                logColor(colors.green, lang.initiatingInCleanMode)
                await sleep(3000)
            } else {
                logFail()
            }
...
async function _sellAll() {
    ...
    if (totalAmount > 0) {
        try {
            ...
            const res = await marketSell(lotQuantity)
            if (res && res.status === 'FILLED') {
                logColor(colors.green, lang.botStopedCorrectlyAllSelled)
            } else {
                logFail()
            }
        } catch (err) {}
...
function logFail() {
    logColor(colors.red, lang.cannotSellInitialBalance)
    logColor(colors.red, lang.mustSellItManuallyOnBinance)
    process.exit()
}
    ...
    try {
        if (currentBalance >= BUY_ORDER) {
            ...
            log(`
                ${lang.buying}${MARKET1}
                ==================
                ${lang.amountIn}${parseFloat(BUY_ORDER.toFixed(2))} ${MARKET2}
                ${lang.amountOut}${BUY_ORDER / price} ${MARKET1}
            `)
            ...
            if (res && res.status === 'FILLED') {
                ...
                logColor(
                    colors.green,
                    lang.purchaseConfirmation(order, totalCost)
                )
                logColor(colors.green, '=============================')
...
const _sell = async price => {
    ...
    if (toSold.length > 0) {
        ...
        try {
            if (totalAmount > 0) {
                log(`${lang.selling}${MARKET1}
                ==================
                ${lang.amountIn}${totalAmount.toFixed(2)} ${MARKET1}
                ${lang.amountOut}${parseFloat(
                        ...
                        for (let j = 0; j < toSold.length; j++) {
                            if (order.id === toSold[j].id) {
                                toSold[j].profit =
                                    ...
                                    parseFloat(toSold[j].amount) *
                                        parseFloat(toSold[j].buy_price)

                                toSold[j].profit -=
                                    order.sell_fee + order.buy_fee
                                toSold[j].sell_fee = parseFloat(
                                    await getFees(res.fills[0])
                                )
                                toSold[j].status = 'sold'
                                orders[i] = toSold[j]
                                store.put(
                                    'fees',
                                    parseFloat(store.get('fees')) +
                                        orders[i].sell_fee
                                )
                                store.put(
                                    'sl_losses',
                                    parseFloat(store.get('sl_losses')) +
                                        orders[i].profit
                                )
                            }
                        }
